 ...
 * 
 * Note: This uses JSON.stringify for cache keys, which:
 * - Fails for non-serializable objects (functions, symbols, circular refs)
 * - May create false cache hits for objects with different property order
 * For production use, consider using a Map with WeakMap for object keys
 */
export const memoize = fn => {
  ...
  return (...args) => {
    try {
      ...
      // If stringification fails, just call the function without caching
      return fn(...args);
    }
  };
};
 ...
 * Note: For large retry counts, consider using iteration instead of recursion
 */
export const retry = (fn, retries = 3, delayMs = 1000) => {
  // Validate that fn returns a promise
  const attempt = () => {
    const result = fn();
    if (!(result instanceof Promise)) {
      return Promise.reject(new Error('retry() requires a function that returns a Promise'));
    }
    return result;
  };
